...
Проект использует [Vitest](https://vitest.dev/) для тестирования бэкенда и фронтенда.
...
```bash
# Бэкенд (auth, access codes, events API)
npm run test:api

# Фронтенд (Button, Input, Modal, authStore)
npm run test:web
```
...
| Область | Тесты | Что проверяется |
|---------|-------|-----------------|
| Auth API | 21 | Регистрация, логин, email-верификация, refresh, сброс пароля |
| Access codes API | 11 | Верификация кода, отзыв, истечение, валидация формата |
| Events API | 13 | CRUD мероприятий, авторизация, проверка доступа |
| UI-компоненты | 21 | Button, Input, Modal — рендер, интерактивность, стили |
| Auth Store | 8 | login, logout, register, refresh, updateTheme |
| **Итого** | **74** | |
